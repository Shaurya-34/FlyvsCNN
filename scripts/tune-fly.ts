// Usage: npx tsx scripts/tune-fly.ts [trials=100] [--three]
...
async function score(tuning: FlyTuning) {
  let collisions = 0;
  let effort = 0;
  let steps = 0;
  for (const seed of TUNE_SEEDS) {
    const r = await runner.benchmark(seed, { kind: 'fly', drive: true, tuning });
